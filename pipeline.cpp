...
int getPc();
void setPc(int _pc);
bool resolveDependence(Instruction& _instr);
Instruction& getNextIns();
Instruction& getCurrentIns();
bool hasNextIns();
int getInstrCount();
void setInstrCount(int _instructionCount);
...
int main(int argc, char* args[]) {
  ...
  cout << "Name: Saeef Ahmad, ID: gij817" << endl;
...
void initialize(int choice) {
  ...
  AbstractStage::setPtrResolveDependence(&resolveDependence);
  AbstractStage::setPtrGetInstrCount(&getInstrCount);
  AbstractStage::setPtrSetInstrCount(&setInstrCount);
  AbstractStage::setPtrGetNextIns(&getNextIns);
  AbstractStage::setPtrGetCurrentIns(&getCurrentIns);
...
bool resolveDependence(Instruction& _instr) {
	...
	bool flag = 0;
	if(instr.isAluReg()){
		if(instr.getDecodeCycle() < regDependence[instr.getArg2()] || instr.getDecodeCycle() < regDependence[instr.getArg3()]){
			flag = 0;
		} else {
			flag = 1;
		}
		if((instr.getDecodeCycle() + 3) > regDependence[instr.getArg1()]){
			regDependence[instr.getArg1()] = instr.getDecodeCycle() + 3;
		}
	} else if(instr.isAluImm()){
		if(instr.getDecodeCycle() < regDependence[instr.getArg2()]){
			flag = 0;
		} else {
			flag = 1;
		}
		...
		if(instr.getDecodeCycle() < regDependence[instr.getArg1()]){
			flag = 0;
		} else {
			flag = 1;
		}
...
Instruction& getCurrentIns() {
  Instruction* ins = imem[(getPc()-4)>>2];
  return *ins;
}
